...
class FinalAccounts extends Component {
    ...
    var objs = search_obj?Object.assign(obj, search_obj):obj
		getData(getRouter(getList("budgetAndFinalAccountsManagementcond","FinalAccounts")), { token: sessionStorage.token,query_condition:objs,data_type:"page_json" }, cb, {});
	...
	freshCardGroup = (newState) => {
		...
		var cb = (route, message, arg) => {
			if (message.error === 0) {
				...
				this.state.page_num,this.state.page_size,this.state.search_message  //刷新项目列表
			}else if(message.error === 2){
				console.log("未登录")
				sessionStorage.logged = false;
				sessionStorage.token="";
					...
					<div id="card_box" onClick={(event) => {
						...
						<div className="selected_scroll_div" style={{ padding: "0 18px" }}>
							...
							{this.state.card_state ?//判断paper是否可见
								...
								<ComponentsList disabled={true} editCardGroupState={this.freshCardGroup} editCardGroupStates={this.freshCardGroup} dataId={this.state.dataId} holdClick={this.onHoldClicks} componentslist =  {this.state.add_button?this.state.add_button:[]} componentsdata = {this.state.edit_project_data} ></ComponentsList > 
						   </div>		
								: ""}
						</div>
					</div>
